...
class AllureListener(object):
    ...
    def start_suitesetup(self, name, attributes):
        start_test_attributes = {
            ...
            'tags': [],
            'id': 's1-s1-t0',
            'longname': BuiltIn().get_variable_value(
                '${SUITE_NAME}'),
            'template': ''
        }
...
#           parameters=[],
    ...
    def start_suite(self, name, attributes):
        ...
        BuiltIn().set_global_variable(
            '${ALLURE}', self.AllureProperties.get_properties())
    ...
    def end_suite(self, name, attributes):
        ...
        # The full check depends on the availability of all the vars which are
        # only available when a Robot file has started.
        IsSuiteDirectory = os.path.isdir(BuiltIn().get_variable_value(
            "${SUITE_SOURCE}"))
    ...
    def start_keyword(self, name, attributes):
        ...
        if(hasattr(self, attributes.get('kwname').replace(" ", "_")) and isinstance(getattr(self, attributes.get('kwname').replace(" ", "_")), collections.Callable)):
            libraryMethodToCall = getattr(self, attributes.get(
                'kwname').replace(" ", "_"))
        ...
        if len(self.stack) > 0:
            if self.stack[-1].title == 'Capture Page Screenshot':
                screenshot = re.search(
                    '[a-z]+-[a-z]+-[0-9]+.png', msg['message'])
                if screenshot:
                    self.attach('{}'.format(
                        screenshot.group(0)), screenshot.group(0))
            if(msg['html'] == 'yes'):
                screenshot = re.search(
                    '[a-z]+-[a-z]+-[0-9]+.png', msg['message'])
...
# Helper functions
    ...
    def attach(self, title, contents, attach_type=AttachmentType.PNG):
        ...
        contents = os.path.join(
            BuiltIn().get_variable_value('${OUTPUT_DIR}'), contents)
    ...
    def Set_Output_Dir(self, name, attributes):
        copy_dir_contents(
            self.AllureProperties.get_property(
                'allure.cli.logs.xml'), attributes['args'][0])
